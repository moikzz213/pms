<template>
  <div>
    <v-app-bar color="white" dense class="elevation-0">
      <v-toolbar-title class="overline">Categories</v-toolbar-title>
    </v-app-bar>
    ...
    <v-container v-else class="mb-3 mx-auto" style="max-width: 1366px">
      ...
      <v-row class="mt-2">
        <v-col cols="12" class="py-0">
          <v-btn small to="/d/admin/categories/new" class="secondary mb-5"
            >New Category</v-btn
          >
        </v-col>
...
export default {
  ...
  methods: {
    ...
    async getAllData(page, orderby = null) {
      ...
      if (this.search) {
        response = await axios.get(
          "/d/admin/categories/fetch-all/" +
            this.search +
            "/?page=" +
            page+"&sort="+sort+"&shows="+this.showPerPage
        );
      } else {
        response = await axios.get(
          "/d/admin/categories/fetch-all/-"  + 
    ...
    searchData: async function () {
      ...
      if (this.search) {
        this.localStorage.setItem("vcategories", this.search);
      }
      ...
      if (this.search && this.search.length > 2) {
        ...
        await axios.get(
          "/d/admin/categories/fetch-all/" +
            this.search +
            "/?page=1&sort="+sort+"&shows="+this.showPerPage
        )
          .then((res) => {
    ...
    onPageChange: function () {
      this.$router.push("/d/admin/categories/page/" + this.page).catch((err) => {});
    },

    clearSearch: function (v) {
      this.localStorage.setItem("vcategories", "");
      this.search = "";
  ...
  created() {
    this.search = this.localStorage.getItem("vcategories");
    if (this.$route.params.page) {
