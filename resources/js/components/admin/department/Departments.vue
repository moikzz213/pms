<template>
  <div>
    <v-app-bar color="white" dense class="elevation-0">
      <v-toolbar-title class="overline">Departments</v-toolbar-title>
    </v-app-bar>
    ...
    <v-container v-else class="mb-3 mx-auto" style="max-width: 1366px">
      ...
      <v-row class="mt-2">
        <v-col cols="12" class="py-0">
          <v-btn small to="/d/admin/departments/new" class="secondary mb-5"
...
export default {
  ...
  methods: {
    ...
    onPageChange: function () {
      this.$router.push("/d/admin/departments/page/" + this.page).catch((err) => {});
    },

    clearSearch: function (v) {
      this.localStorage.setItem("vdepartments", "");
      this.search = "";
  ...
  created() {
    this.search = this.localStorage.getItem("vdepartments");
    if (this.$route.params.page) {
