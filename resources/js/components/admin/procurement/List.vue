          ...
          <v-card class="px-5">
            <v-row>
              <v-col class="col-md-12 col-sm-12 d-flex">
                ...
                <v-autocomplete :items="profiles" @click="fetchProfiles" clearable v-model="dataFilter.user_id" dense
              ...
              <template v-slot:default>
                ...
                <tbody v-if="items && Object.keys(items).length > 0">
                  <tr v-for="(item, index) in items" :key="item.id">
                    ...
                    <td> <v-autocomplete :items="processedBy" 
                      v-model="item.process_by"
                        dense outlined hide-details
                        @change="assignTask(item)"
                        label="Assign To" item-value="id" item-text="name">
                      </v-autocomplete>
  ...
  methods: {
    ...
    async getAllData(page, sort = '', filter = null) {
      ...
      if (this.dataFilter.search) {
        ...
        controller =
          "/d/admin/request/procurement-fetch/-/?page=" + page + "&sort=" + sort + dataComp + dataProcess + dataRequest;
      }
      ...
      await axios
        ...
        .then((response) => {
          ...
          if (nData && nData.length > 0) {
            ...
            let unassign = { id: 'unassign', name: 'Unassign' };
            this.processedBy[nData.length] = unassign;
          }
        });
    ...
    fetchProfiles: async function () {
      if (!this.filterLoaded.requested) {
        await axios
          .get("/d/profile/procurements/profile_users")
          .then((response) => {
            this.profiles = Object.assign([], response.data);
          });
