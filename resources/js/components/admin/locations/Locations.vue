<template>
  <div>
    <v-app-bar color="white" dense class="elevation-0">
      <v-toolbar-title class="overline">Locations</v-toolbar-title>
    </v-app-bar>
    ...
    <v-container v-else class="mb-3 mx-auto" style="max-width: 1366px">
      ...
      <v-row class="mt-2">
        <v-col cols="12" class="py-0">
          <v-btn small to="/d/admin/locations/new" class="secondary mb-5"
            >New Location</v-btn
          >
        </v-col>
...
export default {
  ...
  methods: {
    ...
    searchData: async function () {
      ...
      if (this.search) {
        this.localStorage.setItem("vlocations", this.search);
      }
    ...
    onPageChange: function () {
      this.$router.push("/d/admin/locations/page/" + this.page).catch((err) => {});
    },

    clearSearch: function (v) {
      this.localStorage.setItem("vlocations", "");
      this.search = "";
  ...
  created() {
    this.search = this.localStorage.getItem("vlocations");
    if (this.$route.params.page) {
