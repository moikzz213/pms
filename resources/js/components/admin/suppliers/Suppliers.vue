<template>
  <div>
    <v-app-bar color="white" dense class="elevation-0">
      <v-toolbar-title class="overline">Suppliers</v-toolbar-title>
    </v-app-bar>
    ...
    <v-container v-else class="mb-3 mx-auto" style="max-width: 1366px">
      ...
      <v-row class="mt-2">
        <v-col cols="12" class="py-0">
          <v-btn small to="/d/admin/suppliers/new" class="secondary mb-5">New Supplier</v-btn>
        </v-col>
          ...
          <v-card class="px-5">
            <v-row>
              <v-col class="col-3 col-sm-3 d-flex">
                ...
                <v-autocomplete :items="showRows" v-model="showPerPage" @change="filterRows" dense outlined hide-details
                  label="Entry" class="mt-0 col-md-4 mr-3"></v-autocomplete>
              </v-col>

              <v-spacer></v-spacer>
              <v-col class="col-md-3 col-sm-12">
                <v-text-field v-model="search" append-outer-icon="mdi-magnify" outlined dense
              ...
              <template v-slot:default>
                ...
                <tbody v-if="items && Object.keys(items).length > 0">
                  <tr v-for="(item, index) in items" :key="index">
                    <td> {{ item.title }} </td>
                    <td> {{ item.code }} </td>
                    <td> <div v-if="item.category && item.category.length > 0">
                            <div v-for="cat in item.category" :key="cat.id">
                              {{ cat.title }}
                            </div>
                    </div></td>
                    <td> {{ item.address }} </td>
                    ...
                    <td> {{ item.email }} </td>
                    <td>
                      <v-btn fab x-small depressed @click="editData(item)" class="transparent mr-1">
...
export default {
  ...
  methods: {
    ...
    async getAllData(page, orderby = null) {
      ...
      if (this.search) {
        response = await axios.get("/d/admin/fetch/suppliers/" + this.showPerPage + "/" + this.search + "/" + sort + "/?page=" + page);
      } else {
        response = await axios.get("/d/admin/fetch/suppliers/" + this.showPerPage + "/-/" + sort + "/?page=" + page);
      }
    ...
    searchData: async function () {
      ...
      if (
        ...
        await axios
          .get(
            "/d/admin/fetch/suppliers/" + this.showPerPage + "/" + this.search + "/" + sort
          )
          .then((res) => {
    ...
    onPageChange: function () {
      this.$router
        .push(
          "/d/admin/suppliers/page/" + this.page
        )
        .catch((err) => { });
    },

    clearSearch: function (v) {
      this.localStorage.setItem("vsuppliers", "");
      this.search = "";
  ...
  created() {
    ...
    this.search = this.localStorage.getItem("vsuppliers");
    if (this.$route.params.page) {
